...
LveDevice::LveDevice(LveWindow &window) : window{window} {
  createInstance(); // Init the connection between the Application and Vulkan
  setupDebugMessenger(); // Vulkan as low errors check, so need to have one for debugging
  createSurface(); // Make Vulkan display result in a window
  pickPhysicalDevice(); // Make Vulkan work with the CPU/GPU
  createLogicalDevice(); // What feature the CPU/GPU will use
  createCommandPool(); // Can make commands
}
